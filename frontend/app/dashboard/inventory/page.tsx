...
export default function InventoryPage() {
  ...
  const filtered = items.filter((item) => {
    if (deptFilter !== "ALL" && item.department?.id !== deptFilter)
      return false;
    if (
      search &&
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editTarget) {
        const updated = await api.put<InventoryItem>(
          `/inventory/${editTarget.id}`,
          payload,
        );
        setItems((prev) =>
  ...
  const handleExport = async () => {
    ...
    if (exportParams.dateFrom) {
      data = data.filter(
        (i) =>
          i.purchaseDate &&
          new Date(i.purchaseDate) >= new Date(exportParams.dateFrom),
      );
    }
    if (exportParams.dateTo) {
      ...
      data = data.filter(
        (i) => i.purchaseDate && new Date(i.purchaseDate) <= to,
      );
    }

    data.sort((a, b) => {
      const dir = exportParams.sortDir === "asc" ? 1 : -1;
      if (exportParams.sortBy === "value")
        return (parseFloat(a.value) - parseFloat(b.value)) * dir;
      if (exportParams.sortBy === "name")
    ...
    if (exportParams.groupBy !== "none") {
      ...
      for (const d of data) {
        let key = "";
        if (exportParams.groupBy === "department")
          key = d.department?.name || t("common.none");
        else if (exportParams.groupBy === "warranty") {
          if (!d.warrantyDate) key = t("inventory.exportWarrantyNone");
          else if (isWarrantyExpired(d.warrantyDate))
            key = t("inventory.warrantyExpired");
          else if (isWarrantyExpiringSoon(d.warrantyDate))
            key = t("inventory.warrantyExpiringSoon");
      ...
      for (const [groupKey, groupItems] of groups) {
        ...
        if (exportParams.includeSummary) {
          wsData.push([
            t("inventory.exportGroupTotal"),
            formatInvCurrency(groupTotal),
      ...
      if (exportParams.includeSummary) {
        wsData.push([
          t("inventory.exportGrandTotal"),
          formatInvCurrency(grandTotal),
        ]);
      }
    ...
    if (exportParams.includeSummary) {
      const total = data.reduce(
        (s, d) => s + parseFloat(d.value) * d.quantity,
        0,
      );
      wsData.push([]);
  ...
  const buildInvRow = (d: InventoryItem) => {
    ...
    if (c.purchaseDate)
      row.push(
        d.purchaseDate
          ? new Date(d.purchaseDate).toLocaleDateString("pt-PT")
          : "—",
      );
    if (c.warrantyDate)
      row.push(
        d.warrantyDate
          ? new Date(d.warrantyDate).toLocaleDateString("pt-PT")
          : "—",
      );
    if (c.department) row.push(d.department?.name || "—");
    if (c.purchasedBy) row.push(d.purchasedBy?.name || "—");
    if (c.createdAt)
      row.push(new Date(d.createdAt).toLocaleDateString("pt-PT"));
    return row;
  };
    ...
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {paginate(filtered).map((item) => {
              ...
              return (
                <Card
                  key={item.id}
                  className="bg-background border-border/40 hover:border-primary/40 transition-colors group flex flex-col overflow-hidden"
                >
                  {/* Photo */}
                  ...
                  <CardContent className="flex-1 flex flex-col gap-2 p-3">
                    ...
                    <div className="mt-auto space-y-1.5">
                      ...
                      {item.warrantyDate && (
                        <div
                          className={`flex items-center gap-1 text-xs ${expired ? "text-destructive" : expiringSoon ? "text-orange-500" : "text-muted-foreground"}`}
                        >
                          ...
                          {expiringSoon &&
                            ` (${t("inventory.warrantyExpiringSoon")})`}
                        </div>
                      )}

                      {item.purchasedBy && (
                        <p className="text-xs text-muted-foreground truncate">
                          {t("inventory.byLabel")}: {item.purchasedBy.name}
                        </p>
                      )}
                    </div>
                  </CardContent>
      ...
      <Dialog open={modalOpen} onOpenChange={(o) => !o && setModalOpen(false)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {editTarget ? t("common.edit") : t("inventory.newItem")}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            {error && (
              ...
              <FileUpload
                ...
                onUpload={(result) =>
                  setForm((p) => ({
                    ...p,
                    photoKey: result.key,
                    photoName: result.originalName,
                  }))
                }
                onClear={() =>
                  setForm((p) => ({ ...p, photoKey: "", photoName: "" }))
                }
              />
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setExportParams((p) => ({ ...p, search: e.target.value }))
                }
                placeholder={t("inventory.searchPlaceholder")}
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  type="date"
                  value={exportParams.dateFrom}
                  onChange={(e) =>
                    setExportParams((p) => ({ ...p, dateFrom: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  type="date"
                  value={exportParams.dateTo}
                  onChange={(e) =>
                    setExportParams((p) => ({ ...p, dateTo: e.target.value }))
                  }
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setExportParams((p) => ({ ...p, department: v }))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={exportParams.groupBy}
                  onValueChange={(groupBy) =>
                    setExportParams((previous) => ({
                      ...previous,
                      groupBy: groupBy as typeof exportParams.groupBy,
                    }))
                  }
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    <SelectItem value="none">
                      {t("inventory.exportGroupNone")}
                    </SelectItem>
                    <SelectItem value="department">
                      {t("inventory.exportColDepartment")}
                    </SelectItem>
                    <SelectItem value="warranty">
                      {t("inventory.exportGroupWarranty")}
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select
                  value={exportParams.sortBy}
                  onValueChange={(sortBy) =>
                    setExportParams((previous) => ({
                      ...previous,
                      sortBy: sortBy as typeof exportParams.sortBy,
                    }))
                  }
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    <SelectItem value="createdAt">
                      {t("inventory.exportColCreatedAt")}
                    </SelectItem>
                    <SelectItem value="name">
                      {t("inventory.exportColName")}
                    </SelectItem>
                    <SelectItem value="value">
                      {t("inventory.exportColValue")}
                    </SelectItem>
                    <SelectItem value="purchaseDate">
                      {t("inventory.exportColPurchaseDate")}
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
                ...
                <Select
                  value={exportParams.sortDir}
                  onValueChange={(v) =>
                    setExportParams((p) => ({
                      ...p,
                      sortDir: v as "asc" | "desc",
                    }))
                  }
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    <SelectItem value="desc">
                      {t("inventory.exportDesc")}
                    </SelectItem>
                    <SelectItem value="asc">
                      {t("inventory.exportAsc")}
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 rounded-lg border border-border/40 p-3">
                {[
                  ...
                  {
                    key: "description",
                    label: t("inventory.exportColDescription"),
                  },
                  { key: "value", label: t("inventory.exportColValue") },
                  { key: "quantity", label: t("inventory.exportColQty") },
                  {
                    key: "purchaseDate",
                    label: t("inventory.exportColPurchaseDate"),
                  },
                  {
                    key: "warrantyDate",
                    label: t("inventory.exportColWarranty"),
                  },
                  {
                    key: "department",
                    label: t("inventory.exportColDepartment"),
                  },
                  { key: "purchasedBy", label: t("inventory.exportColBuyer") },
                  {
                    key: "createdAt",
                    label: t("inventory.exportColCreatedAt"),
                  },
                ].map(({ key, label }) => (
                  <label
                    key={key}
                    ...
                    <input
                      ...
                      checked={
                        exportParams.columns[
                          key as keyof typeof exportParams.columns
                        ]
                      }
                      onChange={(e) =>
                        setExportParams((p) => ({
